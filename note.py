...
class Note(pygame.sprite.Sprite):
    def __init__(self,
                 ...
                 sharp_img='Resources/images/sharp_note.png',
                 note_value=60,
                 ):
        super().__init__()
        self.img_offset = 28
        if notes[note_value][2] == True:
            self.sharp = True
            self.image = pygame.image.load(sharp_img)
        else:
            self.sharp = False
...
class Staff():
    ...
    def add_random_note(self, start, stop):
        n = Note(0,0,note_value=random.randint(start,stop))
        self.calculate_pos(n)
        self.notes.add(n)
